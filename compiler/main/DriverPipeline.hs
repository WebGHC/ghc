...
runPhase (RealPhase LlvmLlc) input_fn dflags
  = do
    next_phase <- if fastLlvmPipeline
  ...
  where
    ...
    --
    llvmOpts = case optLevel dflags of
      0 -> "-O1" -- required to get the non-native reg allocator. Passing -regalloc=greedy is not sufficient.
      1 -> "-O1"
      _ -> "-O2"
